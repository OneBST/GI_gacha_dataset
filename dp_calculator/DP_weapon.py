...
plt.title("原神双UP中一把特定武器获取概率", fontproperties=font_title)
...
for each_pos in attention_pos:
    ...
    plt.text(each_pos-0.08, 1625, str(int(each_pos*100))+"%", fontproperties=font, c='lightgray')
for j in range(1, 6):
    ...
    for i in range(1, 1+calc_upper_bound):
        ...
        for each_pos in attention_pos:
            if P_sum[i] >= each_pos >= P_sum[i - 1]:
                plt.text(each_pos-0.06, i, str(i), fontproperties=font, c='#262433')
                if each_pos == 0.5:
                    plt.text(each_pos-0.12, i, str(j)+"精", fontproperties=font, c='#847db3')
